...
## PC1 and PC2 are likely to be dropped because of the big drops around these two compnents.
## Part II
...
# It could be referred that PC1 and PC2 are positively correlated with mean_correct. The correlation between PC1/PC2 and mean_correct will not detected if they are dropped.
...
rownames(F1) <- F1[,1]
F1 <- select(F1, 2:49)
pca.F1 <- prcomp(F1, scale. = TRUE)
plot(pca.F1, type = "lines")
# The part from 5 to 8 will be chosen for analysis. 
DF1 <- as.data.frame(pca.F1$rotation)
##In PC5, programs are more related to art, such as Music, Dance Education, Art Administration, Art Education. PC6 is focus more on Early Childhood Education and Developmental Psychology. PC7 is centered on investigating and measuring how people learn, such as Learning Analytics, Anthropology, Measurement Evaluation and Statistics. PC8 is concerning behavioral or physical related Education, such as Health Education, Behavior Analysis etc.
